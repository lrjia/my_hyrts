...
public abstract class ImmutableGraph<N> extends ForwardingGraph<N> {
   ...
   public static <N> ImmutableGraph<N> copyOf(Graph<N> graph) {
      return (ImmutableGraph)(graph instanceof ImmutableGraph ? (ImmutableGraph)graph : new ImmutableGraph.ValueBackedImpl(GraphBuilder.from(graph), getNodeConnections(graph), (long)graph.edges().size()));
   }
